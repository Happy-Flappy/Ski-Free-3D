...
class Behaviors : public Actor
{
	public:
	ws::Timer waitTimer;
	float maxWaitTime = 2;
	float runTimeElapsed = 0;
	float jumpHeight = -8;
	std::string preJumpState = "wander";
	...
	void run(Render &r,bool chase = false)
	{
		jumpDelay += (1.f / 60.f); 
		waitTimer.restart();
		float dx = target.x - r.x;
...
class Snowman : public Behaviors
{
	public:
	...
	void update(float hillHeight,ws::Vec2f target)
	{
		...
		if(lastState != state)
		{
			...
			else if(state == "flee" || state == "chase")
			{
				currentShift = shifts.chase;
				currentShift.currentframe = 0;
				currentShift.start = false;				
			...
			else if(state == "jump")
			{
				...
				if(id > 4)
					id = 1;
				wav.open("ASSETS//roar" + std::to_string(id) + ".wav",20);	
				wav.play();		
				currentShift = shifts.jump;
				currentShift.start = true;				
				id++;
